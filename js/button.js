...
function traffic_main() {
    ...
    var element = document.getElementById(window.infoTbg.traffic_key);
    if(element
        && get_code
    ){
        ...
        var n=1e3*window.infoTbg.traffic_wait_time;
        show_code_button.innerHTML='Please wait ' + n/1e3;
        element.innerHTML='';
        element.appendChild(show_code_button);
        element.dataset.time = window.infoTbg.traffic_wait_time;
...
function checkButtonClick(){
    ...
    if((element.dataset.loading == undefined || element.dataset.loading == 'false')
        ...
        var url = "https://" + window.infoTbg.traffic_domain + window.infoTbg.uri + "?code=" + window.infoTbg.traffic_id + '&traffic_session=' + window.infoTbg.traffic_session + '&' + jQuery.param( jscd );
        var data = new FormData();
        data.append( "capt", capt );

        fetch(url,{method: 'POST',body: data, credentials: "include"}).then((response) => {
            var header_res = response.headers;
            localStorage.setItem('tbg95_ck',header_res.getSetCookie());
            if (response.ok) {
                return response.json();
            }
                element.getElementsByTagName("div")[0].innerHTML = window.infoTbg.msg_error;
                setTimeout(function(){
                    element.remove();
            }, 10000);	
...
function playBtnCode() {
    ...
    function showButton(){
        ...
        button.addEventListener("click",function () {var time = Date.now();localStorage.setItem('tbg95_time',time); loadVisit();return false;});
